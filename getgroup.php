<?php
	...
	if(isset($_SESSION['token'])){
		...
		if(isset($_GET['_grpid'])){
			...
			$node = $dom->createElement("grpposts");
			$parnode = $dom->appendChild($node);
			$sql="SELECT * FROM grpposts where grpid='$grpid' and ref='0' order by time desc";
			...
			$ref=$row['p_id'];
			$s="select * from grpposts where ref='$ref' order by time;";
			$r=mysql_query($s);
			while ($crow = @mysql_fetch_assoc($r)){
				...
				$cnewnode->setAttribute("grpid", $crow['grpid']);
